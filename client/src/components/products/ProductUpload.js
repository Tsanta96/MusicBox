...
function ProductUpload() {
    ...
    const state = [name, category, description, parseInt(inventoryAmount, 10), parseInt(price, 10), parseInt(weight, 10)];
    ...
    const renderErrors = (errorIndices) => {
        if (errorIndices === []) {
            ...
            setState();
            const errors = errorIndices.map(idx => state[idx].toString() + ' field cannot be blank');
            setErrors(errors);
        }
    }
    ...
    //             // we might have to use a switch here
    //             // Let's try it, lol nvm let's just try it, you take the lead
    //         } else {
    ...
    // Do we have to do something different for int's? like above^ oh yeah prob
    ...
    const uploadFile = async () => {
        ...
        console.log('s3sign mutation: ', response);
        const { signedRequest, url } = response.data.signS3;
        debugger;
        const upload = await uploadToS3(photo, signedRequest)
        ...
        setImageUrl( graphqlResponse.data.createPhoto.id );
    }
                ...
                return (
                    ...
                    <div>
                        ...
                        <div className="product-upload-form-container">
                            <form
                                onSubmit={e => { //Add parse Int on Submit
                                    e.preventDefault();
                                    setState();
